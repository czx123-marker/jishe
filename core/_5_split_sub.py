...
def calc_len(text: str, lang: str = 'en') -> float:
    text = str(text) # force convert
    
    # Only apply complex weighting for specific target languages
    if lang in ['zh', 'ja', 'ko']:
        ...
        # For English and other languages, simple length is sufficient
        return len(text)
...
def split_align_subs(src_lines: List[str], tr_lines: List[str]):
    ...
    target_lang = load_key("target_language") # Get the target language
    ...
    for i, (src, tr) in enumerate(zip(src_lines, tr_lines)):
        ...
        if len(src) > MAX_SUB_LENGTH or calc_len(tr, lang=target_lang) * TARGET_SUB_MULTIPLIER > MAX_SUB_LENGTH:
            to_split.append(i)
...
def split_for_sub_main():
    ...
    for attempt in range(3):  # 多次切割
        ...
        if all(len(s) <= MAX_SUB_LENGTH for s in split_src) and \
           all(calc_len(t, lang=load_key("target_language")) * TARGET_SUB_MULTIPLIER <= MAX_SUB_LENGTH for t in split_trans):
            break
